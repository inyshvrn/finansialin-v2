...
const Chatbot = () => {
  ...
  const handleSend = async () => {
    ...
    const mappedHistory = messages.map((message) => ({
    ...
    try {
      const payload = { message: currentMessage, history: mappedHistory };
      ...
      const botMsg = {
        ...
        text: botText || "Maaf, aku tidak bisa memproses balasan dari server.",
        sender: "bot",
        time: new Date().toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
    ...
    <div className="fixed bottom-8 right-8 z-[100] font-sans">
        ...
        <div className="absolute bottom-20 right-0 w-[320px] h-[480px] bg-white rounded-[28px] shadow-2xl border border-[#E8E2D9] overflow-hidden flex flex-col animate-in slide-in-from-bottom-10 fade-in duration-300">
          <div className="bg-[#1A1A1A] p-5 flex justify-between items-center text-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xs font-black tracking-tighter text-[#FDFCFB]">
                  Finansialin AI
                </h3>
                <div className="flex items-center gap-1.5 opacity-70">
                  <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse" />
                  <span className="text-[9px] font-bold text-[#FDFCFB] uppercase tracking-widest">
                    Active Assistant
                  </span>
                </div>
              </div>
          ...
          <div
            ref={scrollRef}
            className="flex-1 overflow-y-auto p-5 space-y-5 bg-[#FDFCFB]"
          >
            {messages.map((msg) => (
              <div
                key={msg.id}
              ...
              >
                <div className={`max-w-[85%] space-y-1`}>
                  ...
                  <p
                    className={`text-[8px] font-black uppercase text-[#A3A3A3] tracking-widest ${msg.sender === "user" ? "text-right" : "text-left"}`}
                  >
                    {msg.time}
                  </p>
                </div>
      ...
      <button
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 hover:scale-110 active:scale-90 group ${
          isOpen
            ? "bg-white text-[#1A1A1A] rotate-90 border border-[#E8E2D9]"
            : "bg-[#1A1A1A] text-[#FFD600] border-2 border-[#FFD600]/10"
        }`}
      >
        {isOpen ? (
          <X size={24} />
