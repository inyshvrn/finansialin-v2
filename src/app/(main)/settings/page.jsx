  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <div className="space-y-6">
        ...
        <div className="flex w-fit items-center rounded-full border border-[#E8E2D9] bg-white p-1.5 shadow-sm">
          <button
            ...
            className={`h-11 w-40 rounded-full text-xs font-black uppercase tracking-widest transition-all duration-300 ${activeTab === "profile" ? "bg-[#FFD600] text-[#1A1A1A] shadow-sm" : "text-[#7A746E] hover:text-[#1A1A1A]"}`}
          >
            Edit Profile
          </button>
          <button
            onClick={() => setActiveTab("preferences")}
            className={`h-11 w-40 rounded-full text-xs font-black uppercase tracking-widest transition-all duration-300 ${activeTab === "preferences" ? "bg-[#FFD600] text-[#1A1A1A] shadow-sm" : "text-[#7A746E] hover:text-[#1A1A1A]"}`}
          >
            Preferences
          </button>
        </div>
      ...
      <div className="bg-white p-10 lg:p-14 rounded-[32px] border border-[#E8E2D9] shadow-sm min-h-[600px] relative overflow-hidden">
        ...
        <div className="relative z-10">{activeTab === "profile" ? <ProfileForm /> : <Preferences />}</div>
      </div>
    </div>
  );
}
